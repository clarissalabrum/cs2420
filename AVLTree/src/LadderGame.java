...
public class LadderGame {
    ...
    /**
     * Orginial Word Ladder for assignment 0
     * Uses a linked list
...
//                    System.out.println(results.toString());
                    ...
                    if (word.equals(b)) {
                        ...
                        System.out.println("Seeking a solution from " + a + " -> " + b + " Size of List " + size);
                        System.out.println(" [" + tail.next.value + "] Total enqueues " + enqueue );
                        done = true;
                        break;
                    }
    ...
    void AVLfindLadder(String a, String b, ArrayList<String> list){
        int enqueue = 1;
        boolean done = false;
        //create the AVL tree with WordInfo Objects
        AVLTree tree = new AVLTree();
        tree.insert(new WordInfo(a,0,a, b));
        list.remove(a);

        while(!done){
            WordInfo min = (WordInfo) tree.findMin();
            String test = min.word;
            for (int i = 0; i < list.size(); i++) {
                ...
                //if all but one letter matches it is a possible solution
                if (match == (a.length() - 1)) {
                    if (min.history.contains(word)){
                        continue;
                    }

                    //add to AVL WordInfo tree
                    WordInfo sln = new WordInfo(word, min.moves + 1, min.history + " " + word, b);
                    tree.insert(sln);
                    ...
                    if (word.equals(b)) {
                        //count how many moves
                        System.out.println("Seeking a A* solution from " + a + " -> " + b + " Size of List " + size);
                        System.out.println(" [" + sln.history + "] Total enqueues " + enqueue +"\n");
                        done = true;
                        break;
                    }
                }
    ...
    /**
     * For original WordLadder
     * counts number of moves
     * @param phrase word ladder as a string
     * @return number of words/moves
     */
